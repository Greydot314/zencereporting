...
const HealthRing = ({ score, label, percentile }: { score: number; label: string; percentile: number }) => {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <span className="inline-block text-[11px] font-medium px-2 py-0.5 rounded" style={{ backgroundColor: `${color.replace(")", " / 0.15)").replace("hsl(", "hsl(")}`, color }}>
          {label}
        </span>
...
const CampaignsTab = ({ d }: { d: DurationData }) => (
  <div className="space-y-6 pt-4">
    ...
    <HorizontalBars title="Redemption Rate by Campaign Type" items={d.campaigns.bestCampaignType} barColor="hsl(var(--atlas-warning))" />
