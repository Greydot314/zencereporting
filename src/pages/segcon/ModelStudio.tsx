...
import { BusinessModelCatalog } from "@/components/model-studio/BusinessModelCatalog";
import { BusinessModelDetail } from "@/components/model-studio/BusinessModelDetail";
import type { CatalogModel } from "@/data/modelStudioMockData";
import type { BusinessModel } from "@/data/businessModelsMockData";
...
const ModelStudio = () => {
  ...
  const [activeBusinessModel, setActiveBusinessModel] = useState<BusinessModel | null>(null);
  ...
  if (activeBusinessModel) {
    return (
      <div className="flex-1 overflow-y-auto p-6">
        <BusinessModelDetail model={activeBusinessModel} onBack={() => setActiveBusinessModel(null)} />
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">Discover, configure, run, and evaluate AI/ML models — no code required.</p>
      </div>

      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="mb-6">
          <TabsTrigger value="business">Business Models</TabsTrigger>
          <TabsTrigger value="catalog">Segmentation Catalog</TabsTrigger>
        ...
        <TabsContent value="business" className="animate-in fade-in-50 duration-300">
          <BusinessModelCatalog onSelect={setActiveBusinessModel} />
        </TabsContent>
        ...
        <TabsContent value="my-models" className="animate-in fade-in-50 duration-300">
          <MyModels onViewResults={handleViewResults} onNewModel={() => setActiveTab("catalog")} />
        </TabsContent>
      ...
      <ConfigDrawer model={drawerModel} open={drawerOpen} onClose={() => setDrawerOpen(false)} />
    </div>
  );
};
